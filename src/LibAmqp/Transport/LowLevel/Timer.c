...
amqp_timer_t *amqp_timer_initialize(amqp_context_t *context, amqp_timer_expiry_handler_t expiry_handler)
{
...
{
    ev_timer_stop(context->loop, (ev_timer *) timer);
...
void amqp_timer_start(amqp_context_t *context, amqp_timer_t *timer, ev_tstamp delay)
{
    ...
    ev_timer_again(context->loop, (ev_timer *) timer);
}
