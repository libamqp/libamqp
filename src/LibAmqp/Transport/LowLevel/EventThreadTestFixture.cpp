...
{
    EventThreadTestFixture::EventThreadTestFixture() : m_event_thread(0)
    {
    }

    EventThreadTestFixture::~EventThreadTestFixture()
    {
        int thread_allocations_ok = amqp_event_thread_destroy(context, m_event_thread);
        CHECK(thread_allocations_ok);
    }

    void EventThreadTestFixture::basic_event_thread_handler(amqp_event_thread_t *event_thread)
    {
