...
module AttributeTests
  ...
  def mandatory?(f)
    ...
    def optional_primitive?(f)
      primitive?(f) && !mandatory?(f) 
    end 
end
...
class SimpleMapper
  ...
  def output(n, t, o = {})
      puts "    int #{n}_is_null;" if !o[:mandatory]
...
class TypedefedMapper
  ...
  def output(n, t, o = {})
    puts "    int #{n}_is_null;" if @primitive
...
$mappings['handle'] = TypedefedMapper.new('handle', 'uint', TRUE)
$mappings['transfer-number'] = TypedefedMapper.new('sequence_no', 'uint', TRUE)
$mappings['delivery-number'] =  TypedefedMapper.new('sequence_no', 'uint', TRUE)
